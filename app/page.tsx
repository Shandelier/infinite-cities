...
export default function GlobePage() {
  ...
  return (
    <div style={{ position: 'relative' }}>
        ...
        {isMounted && (
          <Suspense fallback={null}>
            <Globe
              ...
              pointAltitude={(p: any) =>
                ((p as LocationPoint).name === 'Warsaw' ? 0.12 : 0.05) +
                0.01 * Math.sin(Date.now() / 200)
              }
              pointColor={(p: any) => (p as LocationPoint).color || '#ffe066'}
              pointRadius={(p: any) =>
                ((p as LocationPoint).size || 0.3) + 0.02 * Math.sin(Date.now() / 200)
              }
              onPointClick={(p) => setSelected(p as LocationPoint)}
              onPointHover={(p) => setHovered(p as LocationPoint | null)}
            />
          </Suspense>
        )}
      </div>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <h2 style={{ fontSize: 20, margin: 0 }}>{selected.name}</h2>
              <button
                onClick={() => setSelected(null)}
                style={{
                  border: 'none',
      ...
      {hovered && (
        <div className="tooltip" style={{ left: mousePos.x, top: mousePos.y }}>
          {hovered.name}
        </div>
      )}
    </div>
